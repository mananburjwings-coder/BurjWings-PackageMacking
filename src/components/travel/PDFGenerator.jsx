...
export default function PDFGenerator({
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      if (formData.name) {
        ...
        pdf.rect(margin + 20, boxY, contentWidth - 40, boxHeight, "F");

        pdf.setDrawColor(6, 182, 212);
        pdf.setLineWidth(0.8);
        pdf.rect(margin + 20, boxY, contentWidth - 40, boxHeight, "S");

        pdf.setTextColor(30, 64, 175);
        pdf.setFontSize(10);
        pdf.setFont("helvetica", "normal");
        pdf.text("PREPARED FOR", pageWidth / 2, boxY + 8, { align: "center" });

        pdf.setFontSize(18);
        pdf.setFont("helvetica", "bold");
        pdf.text(formData.name.toUpperCase(), pageWidth / 2, boxY + 22, {
          align: "center",
        });
      }

      // Destination (BOX STYLE - EXACT SAME AS PREPARED FOR)
      if (formData.destination) {
        const boxY = 175; // Positioned below Prepared For box
        const boxHeight = 30;
        ...
        pdf.setFontSize(10);
        pdf.setFont("helvetica", "normal");
        pdf.text("DESTINATION", pageWidth / 2, boxY + 8, { align: "center" });

        // Destination Value
        pdf.setFontSize(18);
        pdf.setFont("helvetica", "bold");
        pdf.text(formData.destination.toUpperCase(), pageWidth / 2, boxY + 22, {
          align: "center",
        });
      }
      ...
      pdf.setFontSize(28);
      pdf.setFont("helvetica", "bold");
      pdf.text("$" + grandTotal.toLocaleString(), pageWidth / 2, 265, {
        align: "center",
      });
      ...
      pdf.setFontSize(18);
      pdf.setFont("helvetica", "bold");
      pdf.text("TRIP OVERVIEW", pageWidth / 2, yPosition + 10, {
        align: "center",
      });
      yPosition += 25;
      ...
      if (selectedHotels.length > 0) {
        ...
        pdf.setFont("helvetica", "bold");
        pdf.text(
          "LUXURY ACCOMMODATIONS (" + selectedHotels.length + ")",
          pageWidth / 2,
          yPosition + 10,
          { align: "center" },
        );
      ...
      if (selectedActivities.length > 0) {
        ...
        pdf.setFont("helvetica", "bold");
        pdf.text(
          "EXCITING EXPERIENCES (" + selectedActivities.length + ")",
          pageWidth / 2,
          yPosition + 10,
          { align: "center" },
        );
        ...
        for (let i = 0; i < selectedActivities.length; i++) {
          ...
          pdf.setFont("helvetica", "normal");
          pdf.text(
            "Duration: " + activity.duration + "  |  " + activity.groupSize,
            activityDetailX + 2,
            activityCardY + 24,
          );
          ...
          pdf.setFontSize(8);
          pdf.setFont("helvetica", "bold");
          pdf.text("INCLUDED", activityDetailX + 19.5, activityCardY + 41, {
            align: "center",
          });
      ...
      if (selectedTransport.length > 0) {
        ...
        pdf.setFont("helvetica", "bold");
        pdf.text(
          "TRANSPORTATION (" + selectedTransport.length + ")",
          pageWidth / 2,
          yPosition + 10,
          { align: "center" },
        );
      ...
      pdf.setFontSize(14);
      pdf.setFont("helvetica", "bold");
      pdf.text("PACKAGE SUMMARY", pageWidth / 2, yPosition + 15, {
        align: "center",
      });
      ...
      pdf.setFontSize(18);
      pdf.setFont("helvetica", "bold");
      pdf.text("THANK YOU!", pageWidth / 2, yPosition + 15, {
        align: "center",
      });
      ...
      const fileName =
        "Travel-Package-" +
        (formData.name?.replace(/\s+/g, "-") || "Summary") +
        "-" +
  ...
  const isDataComplete =
    formData.name && formData.phone && formData.destination;
  ...
  return (
    ...
    >
      <div className="max-w-4xl mx-auto px-6 text-center">
        ...
        >
          <p>
            PDF shows only the total package price - individual pricing not
            displayed
          </p>
        </motion.div>
      </div>
